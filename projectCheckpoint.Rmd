...
`Age` has slightly more of a right skew but giving it a right log scale made it uniform which is not what we want.
`ValueEUR` did benefit from a log transformation as it fixed it's strong right skew. 
...
**Answer:** It's tough to make out becuase of all the data points but there seems 
...
and then decide if any more variales need to be added from there. 
